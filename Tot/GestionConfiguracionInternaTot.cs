...
namespace Tot
{
    public class GestionConfiguracionInternaTot
    {
        ...
        public void ingresarProcesadorDeLoggeo(ProcesadorLoggeoInferencia procesador)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public ProcesadorLoggeoInferencia extraerProcesadorDeLoggeo()
        {
            ProcesadorLoggeoInferencia encontrado;
            ...
            if (File.Exists(path))
            {
                using (Stream stream = File.OpenRead(path))
                {
                    try
                    {
                        ...
                        encontrado = (ProcesadorLoggeoInferencia)deserializer.Deserialize(stream);
                        stream.Close();
                    }
                    catch (Exception e)
                    {
